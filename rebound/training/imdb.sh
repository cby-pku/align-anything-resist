...
TRAIN_SPLIT='train'
OUTPUT_ROOT_DIR='/mnt/shared-storage-user/zhoujiayi/boyuan/model_results/rebound/imdb/'
...
for MODEL_NAME_OR_PATH in "${MODEL_LIST[@]}"; do
    ...
    for pos_num in "${postive_list[@]}"; do
        TRAIN_DATASETS=/mnt/shared-storage-user/zhoujiayi/boyuan/datasets/preference_datasets/imdb/pos/${pos_num}/train.json
    ...
    for pos_num in "${postive_list[@]}"; do
        for neg_num in "${negative_list[@]}"; do
            FINAL_TRAIN_DATASETS=/mnt/shared-storage-user/zhoujiayi/boyuan/datasets/preference_datasets/imdb/neg/${neg_num}/train.json
